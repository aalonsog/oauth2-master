...
app.get('/', function(req, res){
    ...
    if(!req.session.access_token) {
        res.send("Oauth2 Demo.<br><br><button onclick='window.location.href=\"/auth\"'>Log in with Google</button>");

    // If auth_token is stored in a session cookie it sends a button to get user info
    } else {
        res.send("Successfully authenticated. <br><br> Your oauth access_token: " +req.session.access_token);
    }
});
...
app.get('/auth', function(req, res){
    var path = oa.getAuthorizeUrl(response_type, 'https://www.googleapis.com/auth/gmail.readonly');
    res.redirect(path);
});
